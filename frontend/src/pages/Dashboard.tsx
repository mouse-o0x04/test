...
import { getOrders, getOrderHistory, toggleItemCompleted, toggleItemPrinted, updateOrder } from "../api/orders";
...
export default function Dashboard() {
  ...
  const setStatusFilter = (v: string | null) => ef.updateFilters({ ...ef.filters, status: v });
  ...
  const lowStockItems = warehouse?.filter((w) => w.min_quantity > 0 && w.quantity <= w.min_quantity) ?? [];

  const dateFilteredOrders = allOrders.filter((o) => {
    if (clientFilter && o.client_name !== clientFilter) return false;
    const dateRef = o.deadline || o.created_at;
    if (dateRange?.[0] && dayjs(dateRef).isBefore(dateRange[0].startOf("day"))) return false;
    if (dateRange?.[1] && dayjs(dateRef).isAfter(dateRange[1].endOf("day"))) return false;
    return true;
  });
  ...
  const activeClientNames = [...new Set(activeOrders.map((o) => o.client_name).filter(Boolean))];
  ...
  const statCards = [
    { title: "Активные заказы", value: activeOrders.length, icon: <ShoppingCartOutlined />, color: "#3b82f6", statusKey: null },
    { title: "В работе", value: inProgressOrders.length, icon: <SyncOutlined />, color: statusColorMap["in_progress"] || "#fa8c16", statusKey: "in_progress" },
    { title: "Готовые к выдаче", value: readyOrders.length, icon: <CheckCircleOutlined />, color: statusColorMap["ready"] || "#52c41a", statusKey: "ready" },
    { title: "Забрали", value: deliveredOrders.length, icon: <ClockCircleOutlined />, color: "#8c8c8c", statusKey: "delivered" },
    { title: "Клиенты", value: clients?.length ?? 0, icon: <TeamOutlined />, color: "#8b5cf6", link: "/clients" },
    { title: "Продукты", value: products?.length ?? 0, icon: <ShopOutlined />, color: "#06b6d4", link: "/products" },
    { title: "На складе", value: warehouse?.length ?? 0, icon: <DatabaseOutlined />, color: "#10b981", link: "/warehouse" },
  ];

  const [editingStatusId, setEditingStatusId] = useState<number | null>(null);
  const editingStatus = editingStatusId !== null;
  ...
  const columns = [
    ...
    ...(canViewPrices ? [{ title: "Сумма", dataIndex: "total_price", key: "price", render: (v: number) => <span className="nc-price">{v.toLocaleString()} ₽</span>, sorter: (a: Order, b: Order) => a.total_price - b.total_price }] : []),
    {
      ...
      render: (_: unknown, record: Order) => {
        ...
        return (
          <Tag
            color={statusColorMap[record.status] || "#d9d9d9"}
            style={{ margin: 0, cursor: "pointer" }}
            onClick={(e) => { e.stopPropagation(); setEditingStatusId(record.id); }}
          >
  ...
  return (
    <div style={{ padding: 16 }}>
      <Row gutter={[12, 12]}>
        {statCards.map((card) => {
          const isActive = "statusKey" in card && card.statusKey === statusFilter;
          const cardContent = (
            <Card
              size="small"
              hoverable
              style={{
                ...
                outline: isActive ? `2px solid ${card.color}` : undefined,
              }}
            >
              <Statistic
                title={card.title}
          ...
          if ("link" in card && card.link) {
            return (
              <Col key={card.title} xs={12} sm={12} lg={6}>
                <Link to={card.link}>{cardContent}</Link>
              </Col>
            );
          }

          if ("statusKey" in card) {
            return (
              <Col key={card.title} xs={12} sm={12} lg={6} onClick={() => setStatusFilter(isActive ? null : (card.statusKey ?? null))} style={{ cursor: "pointer" }}>
                {cardContent}
              </Col>
            );
          }
          ...
          <Col xs={12} sm={12} lg={6}>
            <Card size="small" style={{ borderTop: "3px solid #ef4444" }}>
              <Statistic
                ...
                valueStyle={{ fontSize: 24, fontWeight: 600, color: "#ef4444" }}
              />
            </Card>
          </Col>
        )}
        ...
        <Col span={24}>
          <Card
            ...
            title={
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <Typography.Text strong style={{ fontSize: 13 }}>Заказы</Typography.Text>
                <Select
                  allowClear
                  placeholder="Статус"
                  size="small"
                ...
                <Select
                  ...
                  style={{ width: 160 }}
                  options={activeClientNames.map((n) => ({ label: n, value: n }))}
                />
                <RangePicker
                  size="small"
          ...
          >
            <Table
              ...
              columns={columns}
              rowKey="id"
              pagination={{ pageSize: 8, showSizeChanger: false, showTotal: (t) => `Всего: ${t}` }}
              size="small"
              scroll={{ x: "max-content" }}
              onRow={(record) => ({
                onClick: () => { if (!editingStatus) setDetailOrder(record); },
          ...
          <>
            ...
            <Descriptions column={2} size="small">
              <Descriptions.Item label="Создан">
                {detailOrder.created_by_name ? `${detailOrder.created_by_name} (${detailOrder.created_by_role})` : "—"}
              </Descriptions.Item>
