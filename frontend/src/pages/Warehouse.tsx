import { ExclamationCircleOutlined, PlusOutlined, WarningOutlined, DeleteOutlined, ClockCircleOutlined } from "@ant-design/icons";
import {
  Button,
  Card,
  Descriptions,
...
export default function WarehousePage() {
    ...
    },
    {
      ...
      render: (_: unknown, record: WarehouseItem) => (
        <Space>
          ...
          <Button type="link" size="small" icon={<ClockCircleOutlined />} onClick={(e) => { e.stopPropagation(); setAuditEntity({ entityType: "warehouse", entityId: record.id, entityName: getItemName(record) }); setAuditOpen(true); }}>История</Button>
          ...
          <Popconfirm title="Удалить со склада?" onConfirm={() => deleteMutation.mutate(record.id)} onCancel={(e) => e?.stopPropagation()}>
            <Button type="link" danger onClick={(e) => e.stopPropagation()}>Удалить</Button>
          </Popconfirm>
        </Space>
      ),
    },
  ...
  return (
    <>
      ...
      <Table dataSource={filteredData} columns={columns} rowKey="id" loading={isLoading} pagination={{ pageSize: 20, showSizeChanger: true, showTotal: (t) => `Всего: ${t}` }} size="small" rowSelection={{ selectedRowKeys, onChange: setSelectedRowKeys }} onRow={(record) => ({ onClick: () => setWriteoffDrawerItem(record), style: { cursor: "pointer" } })} onChange={(_pagination, filters, sorter) => {
